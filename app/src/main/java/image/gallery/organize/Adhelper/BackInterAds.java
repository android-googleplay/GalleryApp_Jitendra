...
import image.gallery.organize.Activity.QurekaInterActivity;
...
public class BackInterAds {
    ...
    public void loadInterAds(Activity context) {
        if (PowerPreference.getDefaultFile().getBoolean(Constant.AdsOnOff, false) && PowerPreference.getDefaultFile().getInt(Constant.SERVER_BACK_COUNT) > 0 && PowerPreference.getDefaultFile().getBoolean(Constant.BACK_ADS, false)) {
