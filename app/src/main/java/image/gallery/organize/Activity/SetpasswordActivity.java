...
*/
        txtSubmitAns.setOnClickListener(view13 -> {
            if (strings[spinner.getSelectedIndex()].equalsIgnoreCase(strings[0])) {
                Utils.getInstance().showWarning(SetpasswordActivity.this, "Please select valid question.");
                return;
            } else if (editAns.getText() == null || editAns.getText().toString().isEmpty()) {

                Utils.getInstance().showWarning(SetpasswordActivity.this, "Answer should not be empty.");
                return;
            }
