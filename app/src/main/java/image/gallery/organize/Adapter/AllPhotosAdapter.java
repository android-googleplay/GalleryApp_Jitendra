...
public class AllPhotosAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> implements StickyHeaders, StickyHeaders.ViewSetup {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        if (viewType == HEADER_ITEM) {
            ...
            View inflate = LayoutInflater.from(parent.getContext()).inflate(R.layout.layout_ad_home_image, parent, false);
    ...
    public void moreOptionClicked(View v, Activity activity) {
        if (selectedItems.size() > 0) {
            ...
            infoPopup.showAtLocation(v, Gravity.BOTTOM, size.x - v.getWidth(), v.getHeight() + (int) context.getResources().getDimension(R.dimen._40sdp));
    ...
    private class startCopyFiles extends AsyncTask<Void, Void, Void> {
        ...
        protected void onPostExecute(Void unused) {
            ...
            if (isDeleteFromOrigin && Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
                ...
                ((MainActivity) context).runOnUiThread(() -> {
                    Utils.getInstance().showSuccess(((MainActivity) context), "Copied " + selectedItems.size() + " items");
                    folderData.put(file.getName(), copiedlist);
                    Utils.getInstance().showSuccess(context, "Copied " + selectedItems.size() + " items");
                    dialog.dismiss();
                    disableEdit();
