...
public class HideImagesFragment extends Fragment {
    ...
    String[] strings = new String[]{"Select your question", "What's your father's name?", "What's your mother's name?", "What's your favourite movie?", "What's your pets name?", "What's your dream job?", "In which city did your parents meet?"};
       ...
       /* relQue.setOnClickListener(view12 -> {
        ...
        txtSubmitAns.setOnClickListener(view13 -> {
            ...
            if (strings[spinner.getSelectedIndex()].equalsIgnoreCase(strings[0])) {
                Utils.getInstance().showWarning(getContext(), "Please select valid question.");
                return;
            } else if (editAns.getText() == null || editAns.getText().toString().isEmpty()) {
                Utils.getInstance().showWarning(getContext(), "Answer should not be empty.");
                return;
            }
        ...
        if (Utils.getInstance().getPassword(getContext()).equals("")) {
            txtSelectedTitle.setText("Set Password");
        } else {
            txtSelectedTitle.setText("Enter Password");
        }
    ...
    private void unlock() {
        if (selectedItems.size() > 0) {
            ...
            Utils.getInstance().showWarning(getActivity(), "Please select images to unhide");
        }
    }
            ...
            new Handler().postDelayed(() -> getActivity().runOnUiThread(() -> {
                ...
                if (isResetPwd) {
                    if (isEnterAgain) {
                        ...
                        txtTitlePwd.setText("Please Confirm Password");
                        isEnterAgain = false;
                    ...
                    if (enteredPWd.equals(Utils.getInstance().getPassword(getContext()))) {
                        ...
                        txtincorrect.setVisibility(View.VISIBLE);
                        txtincorrect.setText("Password does not match!");
            ...
            for (int i = 0; i < selectedItems.size(); i++) {
                ...
                getActivity().runOnUiThread(() -> {
                    ...
                    if (finalI == selectedItems.size() - 1) {
                        ...
                        Utils.getInstance().showSuccess(getContext(), "Unhided " + selectedItems.size() + " items");
                        dialog.dismiss();
                    }
